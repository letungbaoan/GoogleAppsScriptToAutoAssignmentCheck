function updateCheckboxes() {
  ...
  const dataA = sheetA.getRange(`A${dongDuLieuDauTienSheetA}:${cotCuoiCungCanDocSheetA}` + sheetA.getLastRow()).getValues();
  
  const dataB = sheetB.getRange(`A${dongDuLieuDauTienSheetB}:${cotCuoiCungCanDocSheetB}` + sheetB.getLastRow()).getValues();
  ...
  for (let i = 0; i < dataA.length; i++) {
    ...
    if (studentPassingScores.has(Number(studentId))) {
      sheetA.getRange(cotChuaCheckBox + (i + dongDuLieuDauTienSheetA)).setValue(true);
    }
  }
}
