function updateDropdownTuVungTuan4() { 
  ...
  const dataA = 
  sheetA.getRange(`A${dongDuLieuDauTienSheetA}:${cotCuoiCungCanDocSheetA}${ha
 ngCuoiCungCanDocSheetA}`).getValues(); 
  const dataB = 
  sheetB.getRange(`A${dongDuLieuDauTienSheetB}:${cotCuoiCungCanDocSheetB}` + 
sheetB.getLastRow()).getValues();const studentInfos = new Map(); 
   
dataB.forEach(row => { 
    const studentId = Number(row[cotMSSVSheetB]); 
    const time = new Date(row[cotThoiGianSheetB]); 
    studentInfos.set(studentId, {time}); 
  }); 
  ...
  cell.setBackground('#FFFFFF'); 
      ...
      cell.setDataValidation(SpreadsheetApp.newDataValidation() 
        ...
        .build()); 
      cell.setValue('X'); 
      // Màu hồng cell.setBackground('#FFCFC9'); 
      // Màu xám cell.setBackground(‘#CCCCCC’); 
      cell.setFontColor('#85200c'); 
    } 
  } 
}
